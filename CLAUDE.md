...
- **`app/page.tsx`** assembles the whole page as a top-to-bottom sequence of `<section>` blocks (hero → pain/lead → who-it's-for/not-for → reviews → mechanism → solution reveal → benefits → reviews → screenshot → bonuses → offer/price → proof-of-purchase → urgency/guarantee → reviews → FAQ → final CTA → PS → footer). Section copy lives inline; the bullet arrays (`MAIN_BULLETS`, `BONUS1_BULLETS`, `BONUS2_BULLETS`) sit at the top of the file.
- **Two distinct kinds of social proof, kept separate.** The `Testimonials`/`screen.jpg` blocks sell *results* (the product works). The "Real Orders This Week" section (right after offer/price, `proof1`–`proof5.jpg`) sells *legitimacy*: WhatsApp payment-and-delivery screenshots that kill the "is this real / will I actually receive it" objection at the moment of decision. Do not relabel these as reviews. Buyer phone numbers in those screenshots have their last 3 digits blurred for privacy; keep new ones the same.
...
- **Images** live in `public/images/` as lowercase names (the source assets in the repo root are the originals). The page uses plain `<img>` tags with explicit dimensions and `alt`. The unblurred proof-screenshot originals (`sc.jpg`…`sccccc.jpg` in the repo root) are deliberately left untracked so full customer phone numbers never enter git history; only the blurred `public/images/proof*.jpg` are committed.
